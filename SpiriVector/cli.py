...
def topic_watch(
    ...
    if show_paths is None:
        show_paths = is_wildcard

    console_err.print(f"[dim]Subscribing to: [bold]{topic}[/bold] (Ctrl+C to stop)[/dim]")

    def on_sample(sample):
        key = str(sample.key_expr)
        raw = sample.payload.to_bytes().decode("utf-8").strip()

        if json_output:
            if show_paths:
                line = json.dumps({"path": key, "value": raw}, separators=(",", ":"))
            else:
                line = json.dumps(raw, separators=(",", ":"))
            # JSON gets plain syntax highlighting, no YAML structure
            syntax = Syntax(line, "json", theme="ansi_dark", background_color="default")
            console_out.print(syntax)
        else:
            if show_paths:
                if "\n" in raw:
...
def meta_type_schema(
    ...
    prefix: str = typer.Option(None, "--prefix", "-p", help="Key prefix (defaults to all peers via **)"),
):
    ...
    query_path = f"{base}/sv_type_schema/{_type}"
    console_err.print(f"[dim]Querying: {query_path}[/dim]")
